...
const TouchBarrier: FC<Props> = ({
  width, callValue, putValue, putIsActive, callIsActive, 
}) => {
  ...
  return (
    <>
      {putIsActive && <GenericChartComponent clip={false} svgDraw={renderPut} drawOn={['pan']} />}
      {callIsActive && <GenericChartComponent clip={false} svgDraw={renderCall} drawOn={['pan']} />}
    </>
  );
};
