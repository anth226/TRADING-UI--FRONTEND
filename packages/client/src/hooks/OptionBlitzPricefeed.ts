...
export class OptionBlitzPricefeed {
    ...
    protected async _waitForConnection(socket: WebSocket): Promise<void> {
    ...
    protected _processMessage(evt: MessageEvent, category: string) {
        ...
        if (category === "crypto") {
            const { s: symbol, p: price, q: quantity, t: timestamp } = data;
            if (symbol) {
            ...
            const { s: symbol, b: bid, a: ask, dc: delta, dd, ppms, t: timestamp }: { s: string, b: number, a: number, dc: number, dd:number, ppms:boolean, t: number } = data;
            if (symbol) {
                const eventEmitter = this._subscriptions[normalizeSymbol(symbol)];
                eventEmitter?.event.emit(normalizeSymbol(symbol), {
                    symbol: normalizeSymbol(symbol),
